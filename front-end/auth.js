...
async function parseApiResponse(response) {
    ...
    return {
        message: text ? text.substring(0, 180) : `Unexpected response (${response.status})`
    };
}
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (response.status === 405) {
                return {
                    success: false,
                    message: 'API URL is not pointing to your backend service. Current host is serving static pages.'
                };
            }
