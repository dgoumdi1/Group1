...
public class SessionService {
    ...
    public Session startSession(User user) {
        // Optional: Check if a session is already active to prevent duplicates
    ...
    public Session stopSession(User user) {
        ...
        session.setEndTime(Instant.now()); // Triggers the duration calculation in your entity
        return sessionRepository.save(session);
    }

    public Long getTotalMinutesToday(User user) {
        // Get the very beginning of the current day in UTC
    ...
    public Long getTotalMinutesThisWeek(User user) {
        // Calculate 7 days ago from right now
